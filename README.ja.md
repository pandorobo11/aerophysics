...
入力できます。
wheelにはSphinxのHTMLマニュアルが自動生成されて同梱されるため、追加の
ビルドや環境変数設定なしに、インストール直後からオフラインでGUI内の
ドキュメントを表示できます。
...
wheel build時にHTMLマニュアルを生成して `aerophysics/_docs/` に収録します。
開発中の `docs/_build/html` はプレビューおよびリリース用ドキュメントZIPの
生成に引き続き使用します。
